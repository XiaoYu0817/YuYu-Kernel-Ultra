...
fun HomeScreen(viewModel: HomeViewModel, installVm: InstallViewModel) {
    ...
    var showDeviceDetailDialog by remember { mutableStateOf(false) }
    var ramInfo by remember { mutableStateOf("正在读取...") }
        ...
        if (showDeviceDetailDialog) {
            while (isActive) {
                ...
                if (am != null) {
                    ...
                    val availGB = String.format("%.2f", memInfo.availMem / (1024.0 * 1024.0 * 1024.0))
                    val totalGB = String.format("%.2f", memInfo.totalMem / (1024.0 * 1024.0 * 1024.0))
                    val usedPercent = ((memInfo.totalMem - memInfo.availMem).toDouble() / memInfo.totalMem * 100).toInt()
                    ramInfo = "$availGB GB / $totalGB GB (已用 $usedPercent%)"
                }
                ...
                if (bm != null) {
                    ...
                    try {
                        val tempFile = RandomAccessFile("/sys/class/power_supply/battery/temp", "r")
                        val rawTemp = tempFile.readLine().toDouble() / 10.0
                        tempFile.close()
                        batteryInfo = "$capacity% | 温度: ${rawTemp}℃"
                    } catch (e: Exception) {
                ...
                try {
                    ...
                    val freqKHz = reader.readLine().toLong()
                    reader.close()
                    cpuFreqInfo = "${freqKHz / 1000} MHz"
                } catch (e: Exception) {
                    cpuFreqInfo = "无法读取"
                }
    ...
    if (showDeviceDetailDialog) {
        AlertDialog(
            onDismissRequest = { showDeviceDetailDialog = false },
            title = { Text(text = "系统与实时硬件状态") },
            text = {
                Column(
                    modifier = Modifier
                        .fillMaxWidth()
                    ...
                    Text("实时内存占用: $ramInfo")
                    Text("电池状态: $batteryInfo")
                    Text("CPU0 实时频率: $cpuFreqInfo")
                    HorizontalDivider()
                    Text("设备型号: ${Build.MODEL}")
                    Text("设备代号: ${Build.DEVICE}")
                    Text("品牌: ${Build.BRAND}")
                    Text("制造商: ${Build.MANUFACTURER}")
                    Text("Android 版本: ${Build.VERSION.RELEASE} (API ${Build.VERSION.SDK_INT})")
                    Text("Build ID: ${Build.DISPLAY}")
                    Text("CPU 架构: ${Build.SUPPORTED_ABIS.joinToString(", ")}")
                    Text("内核版本: ${System.getProperty("os.version") ?: "未知"}")
                }
            },
            confirmButton = {
                TextButton(onClick = { showDeviceDetailDialog = false }) {
                    Text("关闭")
                }
            }
        )
    ...
    Scaffold(
        ...
        Column(
            ...
            SystemInfoCard(
                ...
                selinuxStatus = uiState.selinuxStatus,
            )
...
private fun formatUptime(millis: Long): String {
    ...
    return stringResource(CoreR.string.home_uptime_format, days, hours, minutes)
}

@Composable
private fun StatusRowCard(statuses: List<StatusInfo>) {
    Card(
        modifier = Modifier.fillMaxWidth(),
        shape = RoundedCornerShape(24.dp)
    ) {
...
private fun SystemInfoCard(
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = stringResource(CoreR.string.home_sysinfo_title),
                style = MaterialTheme.typography.titleLarge
            )
            Spacer(Modifier.height(12.dp))
            AppDetailRow(
                label = stringResource(CoreR.string.home_device_name),
                value = deviceName.ifBlank { stringResource(CoreR.string.not_available) }
            )
            AppDetailRow(
                label = stringResource(CoreR.string.home_android_version),
                value = androidVersion
            )
            AppDetailRow(
                label = stringResource(CoreR.string.home_kernel_version),
                value = kernelVersion.ifBlank { stringResource(CoreR.string.not_available) }
            )
            AppDetailRow(
                label = stringResource(CoreR.string.home_selinux),
                value = selinuxStatus.ifBlank { stringResource(CoreR.string.not_available) }
            )
        }
    }
}
...
private val developers = listOf(
    ...
    DeveloperInfo("XiaoYu0817 (YuYu)", listOf(
        LinkInfo(CoreR.string.github, CoreR.drawable.ic_github, "https://github.com/XiaoYu0817"),
    )),
)
